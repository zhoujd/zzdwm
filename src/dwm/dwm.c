...
applysizehints(Client *c, int *x, int *y, int *w, int *h, int interact)
{
	...
	*h = MAX(1, *h);
	...
	if (*w < bh)
		*w = bh;
	if (resizehints || ((c->isfloating || !c->mon->lt[c->mon->sellt]->arrange) && floathints)) {
		if (!c->hintsvalid)
			updatesizehints(c);
...
setsmfact(const Arg *arg)
{
	...
	if (!selmon->lt[selmon->sellt]->arrange || selmon->sel->isfloating) {
		...
		if (sf > 0.0) {
			if (selmon->sel->y < selmon->mh - lrpad &&
			    selmon->sel->y + sf < selmon->mh - lrpad)
			...
			if (selmon->sel->y + selmon->sel->h - (selmon->showbar ? bh : 0)  > selmon->my + lrpad &&
			    selmon->sel->y + selmon->sel->h + (selmon->showbar ? bh : 0) + sf > selmon->my + lrpad)
				moveclient(selmon->sel->x, selmon->sel->y + sf, selmon->sel->w, selmon->sel->h);
		}
	} else if (selmon->lt[selmon->sellt]->arrange == zetadeck) {
...
setwfact(const Arg *arg)
{
	...
	if (!selmon->lt[selmon->sellt]->arrange || selmon->sel->isfloating) {
		f = arg->f < 1.0 ? arg->f * 500.0 : arg->f;
		if (f > 0.0) {
			moveclient(selmon->sel->x, selmon->sel->y, MIN(selmon->sel->w + f, selmon->mw * 1.5), selmon->sel->h);
		} else {
...
sethfact(const Arg *arg)
{
	...
	if (!selmon->lt[selmon->sellt]->arrange || selmon->sel->isfloating) {
		sf = arg->sf < 1.0 ? arg->sf * 500.0 : arg->sf;
		if (sf > 0.0) {
			moveclient(selmon->sel->x, selmon->sel->y, selmon->sel->w, MIN(selmon->sel->h + sf, selmon->mh * 1.5));
		} else {
...
setdeflayouts(void)
{
	...
	view(&((Arg) { .ui = 1 << ((deftag - 1) % LENGTH(tags)) }));
}
...
setup(void)
{
	...
	lrpad = barpad ? (barpad * 2) : drw->fonts->h;

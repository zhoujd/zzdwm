...
void editorDrawStatusBar(struct a_buf* ab) {
    ...
    int len = snprintf(status, sizeof(status), " Editing: %.20s ", ec.file_name ? ec.file_name : "*stratch*");
    int col_size = ec.row && ec.cursor_y <= ec.num_rows - 1 ? col_size = ec.row[ec.cursor_y].size : 0;
    int r_len = snprintf(r_status, sizeof(r_status), " %s  %d/%d lines  %d/%d cols ", ec.dirty ? "[+]" : "", ec.cursor_y + 1 > ec.num_rows ? ec.num_rows : ec.cursor_y + 1, ec.num_rows,
